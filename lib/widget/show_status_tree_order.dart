...
class ShowStatusTreeOrder extends StatefulWidget {
  @override
  _ShowStatusTreeOrderState createState() => _ShowStatusTreeOrderState();
}

class _ShowStatusTreeOrderState extends State<ShowStatusTreeOrder> {
  String idUser;
  bool statusOrder = true;
  List<OrderModel> orderModels = List();
  List<List<String>> listMenuTrees = List();
  ...
  ListView buildListViewMenuTree(int index) => ListView.builder(
        shrinkWrap: true,
        physics: ScrollPhysics(),
        itemCount: listMenuTrees[index].length,
        itemBuilder: (context, index2) => Row(
          children: [
            Expanded(
              flex: 3,
              child: Text(listMenuTrees[index][index2]),
            ),
            Expanded(
              flex: 1,
              child: Row(
  ...
  Future<Null> readOrderFromIdUser() async {
    if (idUser != null) {
      String url =
          '${MyConstant().domain}/treeshop/getOrderWhereIdUser.php?isAdd=true&idUser=$idUser';
      ...
      if (response.toString() != 'null') {
        ...
        for (var map in result) {
          ...
          List<String> menuTrees = changeArrey(model.nameTree);
