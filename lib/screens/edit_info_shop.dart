...
class _EditInfoShopState extends State<EditInfoShop> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text('ปรังปรุงรายละเอียดร้าน'),
      ),
    );
  }
  ...
  Future<Null> confirmDialog() async {
    showDialog(
      ...
      builder: (context) => SimpleDialog(
        title: Text('คุณแน่ใจว่าจะปรับปรุงรายละเอียดร้านค้า ?'),
        children: <Widget>[
          Row(
  ...
  Future<Null> editThread() async {
    ...
    String urlUpload = '${MyConstant().domain}/treeshop/saveShop.php';
    await Dio().post(urlUpload, data: formData).then((value) async {
      urlPicture = '/treeshop/Shop/$nameFile';

      String id = userModel.id;
      // print('id = $id');

      String url =
          '${MyConstant().domain}/treeshop/editUserWhereld.php?isAdd=true&id=$id&NameShop=$nameShop&Address=$address&Phone=$phone&UrlPicture=$urlPicture&Lat=$lat&Lng=$lng';
